...
def billingitems():
    ...
    Button(billingsto,width=15,text='Save Bill',command=savebill).grid(row=5,column=6)
...
def savebill():
    ...
    lineadd+="===============================================\n"
    lineadd+="                                  No :%s\n\n" % details[5]
    lineadd+="          GROCERY STORE\n"
    lineadd+="  Chameli Devi Group Of Institutions, Indore\n\n"
    lineadd+="-----------------------------------------------\n"
    if t==1:
        lineadd+="Name: %s\n" % named
        lineadd+="Address: %s\n" % addd
    ...
    details[6]=lineadd
    bill=open(name1.get()+details[5]+".txt",'w')
    bill.write(lineadd)
    bill.close()
...
def dailyincome():
    ...
    Label(dailyinco,width=22,text="Today's Total Income: Rs. "+str(total), bg='black',fg='white').grid(row=1,column=0)
    index=0
    ...
    for i in cur:
        if i[4]==today:
            index+=1
            lb1.insert(index,'Bill No.: '+i[5]+'    : Rs. '+i[3])
    c.commit()
